...
# warn: like check but does not count as an error (optional/environment checks)
warn() {
    description="$1"
    shift
    if "$@"; then
        ...
        echo "[WARN] $description (not counted as error)"
    fi
}
...
warn  "Password authentication disabled" grep -q '^PasswordAuthentication no' /etc/ssh/sshd_config
...
# ------------------------------
# SSH key authentication (warnings — requires pubkey setup)
# ------------------------------
warn "authorized_keys file exists"    test -f "/home/$USER_NAME/.ssh/authorized_keys"
warn "authorized_keys is non-empty"   test -s "/home/$USER_NAME/.ssh/authorized_keys"
...
# Allow root to read the repo (safe.directory) before checking commits
git config --global --add safe.directory "$BASE_DIR" 2>/dev/null || true
check "Baseline commit exists" bash -c "git -C $BASE_DIR log --oneline | grep -q 'Baseline administrative structure'"
